...
async fn valid_batch_is_stored(pool: PgPool) {
    ...
    let n: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM events")
        ...
        .unwrap();
    assert_eq!(n, 1);
    let name: String = sqlx::query_scalar("SELECT event_name FROM events")
        .fetch_one(&pool)
        .await
        .unwrap();
...
async fn unknown_app_is_rejected_whole(pool: PgPool) {
    ...
    body["resourceLogs"][0]["resource"]["attributes"][0]["value"]["stringValue"] =
        "evil-app".into();
    let (status, resp) = common::send_json(&app, "POST", "/v1/logs", Some(body), None).await;
...
async fn bad_record_yields_partial_success(pool: PgPool) {
    ...
    body["resourceLogs"][0]["scopeLogs"][0]["logRecords"]
        .as_array_mut()
        .unwrap()
        .push(bad);
    let (status, resp) = common::send_json(&app, "POST", "/v1/logs", Some(body), None).await;
