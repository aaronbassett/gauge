...
- [x] **Step 1: Write the failing serde round-trip test** in `request.rs` `#[cfg(test)]`:
...
- [x] **Step 2: Run it to confirm it fails to compile** (no `Avg` variant yet)

Run: `cargo test -p gauge-query measure_serde_simple_and_aggregate`
Expected: FAIL — `no variant named Avg` / `alias` arity.

- [x] **Step 3: Replace the `Measure` enum** in `request.rs`. Delete the existing `#[derive(... )] pub enum Measure { Count, UniqueInstalls, UniqueSessions }` and its `impl` and replace with:
...
- [x] **Step 4: Run the gauge-query test to confirm it passes**

Run: `cargo test -p gauge-query measure_serde_simple_and_aggregate`
Expected: PASS.

- [x] **Step 5: Add the validation rule + error variant.** In `validate.rs`, add to `enum QueryError`:
...
- [x] **Step 6: Add a validation test** in `validate.rs` tests (create a `#[cfg(test)] mod tests` if absent — there is none today, so add one):
...
- [x] **Step 7: Add `numeric_field_expr`, `ColKind::Float`, and `float_value` to `sqlbuild.rs`.** Add `use serde_json::Value;` near the top imports. Add the `Float` variant to `enum ColKind` (after `TimeBucket`). Add these functions next to `field_expr`:
...
- [x] **Step 8: Replace the measures loop** in `sqlbuild::build` (currently the `for m in &req.measures { ... }` block) with:
...
- [x] **Step 9: Decode the Float column.** In `routes/query.rs`, change `let v = match col.kind {` to `let v = match &col.kind {` and add an arm after `ColKind::TimeBucket => ...`:
...
- [x] **Step 10: Fix the `Measure`-is-no-longer-`Copy` site** in `mcp/tools.rs` `top_events_query`. Replace the `QueryRequest { ... }` construction so the alias is computed before `measure` is moved:
...
- [x] **Step 11: Add a snapshot test for aggregate SQL** in `sqlbuild.rs` tests:
...
- [x] **Step 12: Run tests, accept the snapshot, lint**
...
- [x] **Step 1: Write the failing validation + serde test** in `validate.rs` tests:
...
- [x] **Step 2: Run to confirm failure** (no `FilterOp::Gt`)

Run: `cargo test -p gauge-query numeric_filter_requires_num_and_attr`
Expected: FAIL — `no variant named Gt`.

- [x] **Step 3: Extend `FilterOp` and `FilterValue`** in `request.rs`:
...
- [x] **Step 4: Add the validation arms.** In `validate.rs`, inside the `match (f.op, &f.value)` block, add these arms **before** the existing `(FilterOp::Eq | FilterOp::Neq, _)` catch-all:
...
- [x] **Step 5: Run the gauge-query tests**

Run: `cargo test -p gauge-query` → Expected: PASS.

- [x] **Step 6: Add `Bind::Float` and the server filter arms.** In `sqlbuild.rs`, add `Float(f64)` to `enum Bind` (after `Time`). In `build`, add these arms to the `match (f.op, f.value.as_ref())` block **before** `_ => unreachable!`:
...
- [x] **Step 7: Bind floats at execution.** In `routes/query.rs`, add to the `q = match b { ... }` block:

```rust
            Bind::Float(f) => q.bind(*f),
```

- [x] **Step 8: Add a numeric-filter snapshot + extend the no-leak test** in `sqlbuild.rs` tests:
...
- [x] **Step 9: Run, accept snapshot, lint, commit**
...
- [x] **Step 1: Write the failing `bucket_labels` test** in `validate.rs` tests:
...
- [x] **Step 3: Add `bucket_labels` to `validate.rs`** (top-level `pub fn`):
...
- [x] **Step 4: Add `Dimension`, `BucketSpec`, `MAX_BUCKET_EDGES` and switch the field type** in `request.rs`. Add near `DEFAULT_LIMIT`:
...
- [x] **Step 5: Add a dimension serde + backward-compat test** in `request.rs` tests:
...
- [x] **Step 6: Add edge + duplicate-alias validation.** In `validate.rs` add error variants:
...
- [x] **Step 7: Add a bucket-validation test** in `validate.rs` tests, then run:
...
- [x] **Step 8: Add response meta types** in `response.rs`:
...
- [x] **Step 9: Update `lib.rs` re-exports.** Add `Dimension`, `BucketSpec`, `MAX_BUCKET_EDGES` to the `request::{...}` re-export; add `QueryMeta, BucketMeta` to the `response::{...}` re-export; add `bucket_labels` to the `validate::{...}` re-export:
...
- [x] **Step 10: Server — add `ColKind::Bucket`, `Bind::FloatArr`, `bucket_value`, `BuiltQuery.bucket_meta`.** In `sqlbuild.rs`:
...
- [x] **Step 11: Rewrite the dimensions loop and default order** in `sqlbuild::build`. First add `let mut bucket_meta: Vec<BucketMeta> = Vec::new();` near the other `let mut` locals. Replace the `for d in &req.dimensions { ... }` block with:
...
- [x] **Step 12: Decode the Bucket column and attach meta** in `routes/query.rs`. Add this arm to the `match &col.kind` block:
...
- [x] **Step 15: Run, accept snapshot, lint, commit**
...
- [x] **Step 1: Write the failing schema test** in `meta.rs` (add a `#[cfg(test)] mod tests`):
...
- [x] **Step 3: Add the field** to `struct AppMeta` in `meta.rs`, after `attribute_keys`:
...
- [x] **Step 4: Populate it in the server meta route.** In `routes/meta.rs`, after the `keys` query, add:
...
- [x] **Step 1: Add a CLI parse test** in `query_cmd.rs` tests:
...
- [x] **Step 2: Update the README Query DSL section.** Replace the `**Measures:**`, `**Dimensions:**`, and `**Filters:**` bullets (around `README.md:239-241`) with:
...
- [x] **Step 3: Add a worked numeric example** after the existing query example fence (around `README.md:265`):
...
- [x] **Step 1: Write a failing render test** in `render.rs` tests — an aggregate/bucket query should NOT suggest the count-only trend action:
...
- [x] **Step 4: Extend the `query_telemetry` tool description** in `server.rs`. Replace the doc-comment above `pub async fn query_telemetry` with:
...
- [x] **Step 1: Write the failing builder test** in `tools.rs` tests:
...
- [x] **Step 3: Add params + builder** in `tools.rs`. Add a helper to turn a bare attr key into a `Field`, then the param struct and builder:
...
- [x] **Step 4: Add the projector** in `render.rs`:
...
- [x] **Step 5: Register the output schema.** In `schemas.rs`, add `"numeric_stats"` and `"numeric_histogram"` to the `query_envelope_schema` arm of `schema_for` (both return the query envelope shape):
...
- [x] **Step 6: Add the tool** in `server.rs`. Add the import `numeric_stats_query, NumericStatsParams` to the `use crate::mcp::tools::{...}` and `project_numeric_stats` to the `use crate::mcp::render::{...}`. Add the method inside the `#[tool_router] impl GaugeMcp` block:
...
- [x] **Step 7: Run, lint, commit** (the tool-count test still expects 5 here and will be updated in Task 8; temporarily it will list 6 — update the literal `5` in `server.rs` test `tool_list_has_annotations_and_output_schemas` to `6` and add `"numeric_stats"` to the schema-checking name list):
...
- [x] **Step 3: Add params + builder** in `tools.rs` (add `BucketSpec, Dimension` to the `use gauge_query::{...}` import):
...
- [x] **Step 5: Add the tool + update the tool-count test** in `server.rs`. Add `numeric_histogram_query, NumericHistogramParams` to the tools import and `project_numeric_histogram` to the render import. Add the method:
...
- [x] **Step 1: Write the failing test** in `data.rs` (add a `#[cfg(test)] mod tests`):
...
- [x] **Step 3: Implement the helpers** in `data.rs`:
...
- [x] **Step 1: Write the failing test** in `app.rs` (add a `#[cfg(test)] mod tests`):
...
- [x] **Step 3: Extend the Explore state + measures** in `app.rs`. Add a numeric-measure list and a base index, and a selected numeric attr:
...
- [x] **Step 4: Rebuild `explore_request`** in `app.rs` to emit a numeric-aggregate measure when a numeric measure is selected:
...
- [x] **Step 5: Let the user cycle the numeric attr.** In `app.rs` `on_key`, add a binding (only on the Explore page) to cycle `numeric_attr` through the union of `numeric_attribute_keys` from the current snapshot. Add a key, e.g. `n`:
...
- [x] **Step 6: Show the selection** in `ui.rs` `render_explore` picker line. Replace the `picker` paragraph text with:
...
- [x] **Step 1: Write the failing test** for the histogram fetch's query shapes in `data.rs` tests (we test the request builders, not the network):
...
- [x] **Step 3: Add the request builders + async fetch** in `data.rs` (add `BucketSpec, Dimension` to the `use gauge_query::{...}` import):
...
- [x] **Step 4: Add histogram state + trigger** in `app.rs`. Add to `struct ExploreState`:
...
- [x] **Step 5: Wire the async probe+bucket** in `run.rs`. Add a `Msg` variant:
...
- [x] **Step 6: Render the histogram** in `ui.rs` `render_explore`. When `app.explore.histogram` is `Some`, draw a horizontal bar chart instead of the JSON lines. Replace the `match &app.explore.result { ... }` block with:
...
- [x] **Step 1: Full workspace test + lint**
...
- [x] **Step 2: Confirm the snapshots are committed and accurate**

Run: `git status --porcelain crates/gauge-server/src/snapshots`
Expected: empty (all `.snap` files committed; no stray `.snap.new`).

- [x] **Step 3: Spec acceptance-criteria walk-through.** Re-read §9 of the spec and confirm each item maps to a task: bucket (T3), aggregates incl. percentiles (T1), numeric filters (T2), graceful exclusion (T1/T2/T3 via the shared cast + IS NOT NULL), no value leakage (T2/T3 no-leak asserts), snapshots (T1/T2/T3), CLI docs (T5), MCP description + two tools + render (T6/T7/T8), TUI numeric + histogram (T9/T10/T11), `numeric_attribute_keys` (T4). Note any gap and add a follow-up task.

- [x] **Step 4: Optional fact-check of the README example**
...
- [x] **Step 5: Tick this plan's checkboxes and commit the docs sync**
...
- [x] **Step 6: Finish the branch.** Use the `superpowers:finishing-a-development-branch` skill to choose how to integrate (open a PR referencing issue #22, merge, or keep iterating).
